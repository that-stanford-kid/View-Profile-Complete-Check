Freight & 3PL - Multi Algorithmic Design in R
Patrick C. ONeil
...
require(openxlsx) require(lubridate) require(reshape2) require(rms) require(Hmisc) require(tidyverse) require(kableExtra) require(glmnet) require(stringr) dat <- read.csv(“freight proj.csv”, header = TRUE) d1 <- dat[, 1:22] d2 <- dat[, 23:36] d3 <- dat[, 37:56]
...
$ Low.Rate.Per.Mile.van. = 1.596 - 0.00145Miles.Estimate + 0.000816Median.Total.Without.Fuel.surcharge.LineHaul.Van$
...
coef(dd1)
$ Median.Rate.Per.Mile.Van = 1.788 - 0.00153Miles.Estimate + 0.000854Median.Total.Without.Fuel.surcharge.LineHaul.Van$
Estimate.Average.Rate.Per.Mile.Van(Error Type -> prediction(s))
x <- as.matrix(d2[, c(6, 11:13)])
y3 <- as.matrix(d2[, 9])
...
d2 %>%
  group_by(Lane.Pairs) %>%
  summarise( mean(pred, na.rm = T))
$ Estimate.Average.Rate.Per.Mile.Van = 1.788 - 0.00151Miles.Estimate + 0.000846Median.Total.Without.Fuel.surcharge.LineHaul.Van$
High.Rate.Per.Mile.Van (upper ceiling(s))
x <- as.matrix(d2[, c(6, 11:13)])
y4 <- as.matrix(d2[, 10])
...
coef(dd1)
$ High.Rate.Per.Mile.Van = 1.98 - 0.0016Miles.Estimate + 0.00089Median.Total.Without.Fuel.surcharge.LineHaul.Van$
Load Data
Variable Cluster - Van(Dry)
...
Variable Cluster - Reef(Refrigerated)
...
plot(tr)
Random Forest - Reef(Refrigerated)
...
Decision Tree - Reef(Refrigerated)
